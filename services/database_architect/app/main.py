...
from services.database_architect.app.task_processor import process_schema_task
from services.database_architect.app.core.config import setup_logging
from services.database_architect.app.core.exceptions import BaseAgentException
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Database Architect Agent is starting up.")
    subscriber_task = asyncio.create_task(async_redis_subscriber())
    yield
    logger.info("Database Architect Agent is shutting down.")
...
async def async_redis_subscriber():
    """Listens for tasks on the 'database_architect_tasks' channel."""
    ...
    while True:
        try:
            ...
            logger.info("Received new database design task.", extra={"props": {"task_id": task_data.get("task_id")}})

            await process_schema_task(task_data, redis_client)
...
@app.get("/health")
def read_health():
    """Health check endpoint for the Database Architect Agent."""
    return {"status": "ok"}
