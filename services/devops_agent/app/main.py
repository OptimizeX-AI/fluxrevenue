...
async def lifespan(app: FastAPI):
    ...
    logger.info("DevOps Agent is starting up.")
    subscriber_task = asyncio.create_task(async_redis_subscriber())
    yield
    logger.info("DevOps Agent is shutting down.")
...
async def async_redis_subscriber():
    """Listens for tasks on the 'devops_agent_tasks' channel."""
    ...
    while True:
        try:
            ...
            await process_devops_task(task_data, redis_client)
