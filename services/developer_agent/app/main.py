...
# Setup structured logging according to the ecosystem standard
setup_logging()
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Developer Agent is starting up.")
    subscriber_task = asyncio.create_task(async_redis_subscriber())
    yield
    logger.info("Developer Agent is shutting down.")
...
async def async_redis_subscriber():
    """Listens for tasks on the 'developer_agent_tasks' channel."""
    ...
    while True:
        try:
            ...
            # Delegate processing to the task processor
            await process_development_task(task_data, redis_client)
