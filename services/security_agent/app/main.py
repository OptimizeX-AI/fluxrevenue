...
from services.security_agent.app.task_processor import process_security_task
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Security Agent is starting up.")
    subscriber_task = asyncio.create_task(async_redis_subscriber())
    yield
    logger.info("Security Agent is shutting down.")
...
async def async_redis_subscriber():
    """Listens for tasks on the 'security_agent_tasks' channel."""
    ...
    while True:
        try:
            ...
            logger.info("Received new security task.", extra={"props": {"task_id": task_data.get("task_id")}})

            await process_security_task(task_data, redis_client)
...
@app.get("/health")
def read_health():
    """Health check endpoint for the Security Agent."""
    return {"status": "ok"}
