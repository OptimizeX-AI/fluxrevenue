...
from services.code_reviewer.app.core.config import setup_logging
from services.code_reviewer.app.core.exceptions import BaseAgentException
from services.code_reviewer.app.task_processor import process_review_task
...
AGENT_VERSION = "1.0.0"
AGENT_CAPABILITIES = ["static_analysis", "code_quality_check", "best_practices_enforcement"]
AGENT_SUPPORTED_LANGUAGES = ["python", "javascript"]
...
def task_consumer_callback(message: dict):
    ...
    try:
        ...
        logger.info("Received new code review task.", extra={"props": {"task_id": task_data.get("task_id")}})

        # This agent's task processor is async
        asyncio.run(process_review_task(task_data, None)) # Passing None for redis_client
...
@app.get("/health")
def read_health():
    """Health check endpoint for the Code Reviewer Agent."""
    return {"status": "ok"}
