...
from services.agent_manager.app.core.exceptions import (
    BaseAgentException, ProjectNotFoundError, TaskValidationError, InfrastructureError
)
...
async def async_handle_project_creation_task(message: dict):
    # ... (rest of the function is the same, just keeping it clean)
        ...
        try:
            ...
            if execution_plan:
                ...
                for task in initial_tasks:
                    ...
                    msg = RabbitMQClient.create_message(source_agent='agent_manager', target_agent=agent_name, task_type='execute_task', payload=task_to_dispatch)
        ...
        try:
            ...
            if newly_dispatchable_tasks:
                ...
                for task in newly_dispatchable_tasks:
                    ...
                    msg = RabbitMQClient.create_message(source_agent='agent_manager', target_agent=task['agent'], task_type='execute_task', payload=task_to_dispatch)
                    rabbitmq_client.publish_message(dispatch_queue, msg)
...
async def handle_rejection(session: AsyncSession, project: Project, failed_task_id: int, review_artifact: dict):
    ...
    remediation_description = (f"Remediation required. The '{failed_task['agent']}' agent rejected the previous submission. "
    ...
    remediation_task = {"task_id": remediation_task_id, "agent": "developer_agent", "description": remediation_description, "depends_on": [failed_task_id]}
    ...
    message = RabbitMQClient.create_message(source_agent='agent_manager', target_agent=remediation_task['agent'], task_type='remediation_task', payload=task_to_dispatch)
